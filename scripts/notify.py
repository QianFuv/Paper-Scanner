"""
Select and push updated articles with OpenRouter and PushPlus.
"""
...
DEFAULT_ABSTRACT_PREVIEW_LENGTH = 1000
...
class Subscriber:
    ...
    Args:
        ...
        name: Display name.
        to: Enterprise WeChat user id list.
        keywords: Keyword preferences.
        directions: Direction preferences.
        excluded_keywords: Exclusion terms.
...
class NotificationGlobal:
    ...
    Args:
        openrouter_api_key: OpenRouter API key used for AI selection.
        pushplus_token: PushPlus token for message sending.
...
class OpenRouterSelector:
    """
    OpenRouter client for structured article selection.
    ...
    def select_articles(
        ...
        user_payload = {
            "subscriber": {
                ...
                "excluded_keywords": subscriber.excluded_keywords,
            },
            "selection_rules": {
                ...
                "avoid": [
                    "Items dominated by excluded keywords",
...
class PushPlusClient:
    ...
    def send(
        ...
        Args:
            ...
            to: Enterprise WeChat recipient ids.
...
def parse_subscriber(item: dict[str, Any]) -> Subscriber:
    ...
    if not to_value:
        raise ValueError(f"Subscriber {subscriber_id} missing to value")
    ...
    return Subscriber(
        ...
        excluded_keywords=to_string_list(item.get("excluded_keywords")),
        topic=topic,
        template=template,
    )
...
def load_subscriptions(
    ...
    openrouter_api_key = str(global_obj.get("openrouter_api_key") or "").strip()
    openrouter_api_key_env = str(global_obj.get("openrouter_api_key_env") or "").strip()
    if not openrouter_api_key and openrouter_api_key_env:
        openrouter_api_key = str(os.getenv(openrouter_api_key_env, "")).strip()
        if not openrouter_api_key:
            openrouter_api_key = openrouter_api_key_env
    if not openrouter_api_key:
        openrouter_api_key = str(os.getenv("OPENROUTER_API_KEY", "")).strip()
    if not openrouter_api_key:
        raise ValueError("Global OpenRouter API key is required")

    pushplus_token = str(global_obj.get("pushplus_token") or "").strip()
    pushplus_token_env = str(global_obj.get("pushplus_token_env") or "").strip()
    if not pushplus_token and pushplus_token_env:
        pushplus_token = str(os.getenv(pushplus_token_env, "")).strip()
        if not pushplus_token:
            pushplus_token = pushplus_token_env
    if not pushplus_token:
        pushplus_token = str(os.getenv("PUSHPLUS_TOKEN", "")).strip()
    if not pushplus_token:
        raise ValueError("Global PushPlus token is required")
    ...
    defaults = NotificationDefaults(
        ...
        openrouter_model=str(
            defaults_obj.get("openrouter_model") or DEFAULT_OPENROUTER_MODEL
        ).strip(),
...
def extract_response_payload(response_json: dict[str, Any]) -> dict[str, Any]:
    """
    Extract structured payload from OpenRouter response.

    Args:
        response_json: OpenRouter response JSON.
    ...
    if not isinstance(first_choice, dict):
        raise ValueError("OpenRouter response has invalid choice item")
    ...
    if not isinstance(content, str):
        raise ValueError("OpenRouter message content is invalid")
...
def is_excluded(candidate: ArticleCandidate, excluded_terms: list[str]) -> bool:
    """
    Check whether candidate matches any excluded term.

    Args:
        candidate: Candidate article.
        excluded_terms: Exclusion terms.

    Returns:
        True if excluded term is matched.
    """
...
def apply_selection_rules(
    ...
    for selection in selection_result.selections:
        ...
        if candidate is None:
            continue
        if is_excluded(candidate, subscriber.excluded_keywords):
            continue
...
def build_markdown_content(
    ...
    if summary:
        base_lines.extend(["", f"{summary}"])

    article_sections: list[str] = []
    for index, item in enumerate(selections[:MAX_ARTICLES_PER_PUSH], start=1):
        candidate = candidates_by_id[item.article_id]
        display_abstract = truncate_text(
            candidate.abstract,
            DEFAULT_ABSTRACT_PREVIEW_LENGTH,
        )
        ...
        article_sections.append(
            "\n".join(
                [
                    ...
                    f"- DOI: {display_doi}",
                    f"- Abstract: {display_abstract or 'N/A'}",
                ]
            )
        )

    def render_content(sections: list[str]) -> str:
        """
    ...
    selected_sections = article_sections[:]
    content = render_content(selected_sections)
    while len(content) > MAX_PUSH_CONTENT_LENGTH and selected_sections:
...
def run_notification(args: argparse.Namespace) -> int:
    ...
    with sqlite3.connect(db_path) as connection:
        ...
        model = args.openrouter_model or defaults.openrouter_model
        ...
        try:
            for subscriber in subscribers:
                try:
                    selection_result = selector.select_articles(
                        subscriber,
                        defaults,
                    ...
                    if args.dry_run:
                        ...
                        message_id = push_client.send(
                            token=global_config.pushplus_token,
                            title=message_title,
                            content=content,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Select and push updated articles with OpenRouter and PushPlus"
    )
    parser.add_argument(
        "--db",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Retry count for OpenRouter and PushPlus calls.",
    )
    parser.add_argument(
